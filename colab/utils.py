...
def accuracy(pred, label, threshold=0.5):
    ...
    """
    pred_integer = (pred > threshold).type(torch.LongTensor).to(device)
    accu = (label == pred_integer).sum() / torch.ones(label.shape, device=device).sum()
...
def get_pos_neg_edges_for_user(edges, users, u, unique_movies_set, neg_sample_size=10):
    ...
    neg_movies = torch.tensor(neg_movies, device=device)

    user_tensor = torch.tensor([u], device=device).repeat(neg_sample_size)
